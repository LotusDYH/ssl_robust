...
class GreedyAttack(BaseAttack):
    ...
    def get_grad(self, input_ids, attention_mask):
        ...
        """
        embedding_layer = self.model.encoder_q.get_input_embeddings()
        linear_layer = self.model.encoder_q_linear
        ...
        with torch.enable_grad():
            output, target, outputs_z = self.model(input_ids=input_ids, attention_mask=attention_mask, update=False)
            loss = self.criterion(output, target.to(self.device))
            self.loss_set.append(loss.cpu().item())
            loss.backward()
    ...
    def split_forward(self, input_ids, attention_mask):
        ...
        with torch.no_grad():
            for idx in range(len(input_ids_splits)):
                outputs_z = self.model.get_latent_space(input_ids=input_ids_splits[idx],
    ...
    def adv_attack_samples(self, model, device, input_ids):
